...
调度扩展研究一个具体问题：**研究智能体的并发名额有限时，预测任务长度会怎样影响等待时间？** 项目包括可解释的长度估计规则、先来先服务和预测短任务优先两种执行策略，以及可复现的任务时长预测误差仿真。这是使用 AI 辅助开发的本科原型，没有复现 Prompt2Length 或 JDPMHF，也不是 GPU 调度器。
...
长度估计规则只读取问题措辞，输出的是**相对单位**，不是准确的 token 数或毫秒数。规则尚未校准，可能出现误差。独立仿真器研究受控的时长预测误差，不评价该文字规则的预测准确率。详细信息见[实验设计与结果](docs/EXPERIMENTS.md)和[中文学习笔记](docs/LEARNING_GUIDE_ZH.md)。
...
- 项目尚未进行真实 GPU 实验、训练预测模型或回答质量基准测试，也没有声称获得真实加速效果。

## 后续实验

1. 收集各角色的真实调用记录，使用训练集拟合小型时长预测器，并在独立测试集上评价。
